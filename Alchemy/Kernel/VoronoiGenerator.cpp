...
bool VoronoiDiagramGenerator::generateVoronoi(VFLOAT *xValues, VFLOAT *yValues,  int numPoints, VFLOAT minX, 
											  VFLOAT maxX, VFLOAT minY, VFLOAT maxY, VFLOAT minDist, bool genVertexInfo)
{
	cleanup();
	cleanupEdges();
	int i;
	...
	saved_sites = (struct Site *) myalloc(nsites*sizeof( *sites));
	...
	for(i = 0; i< nsites; i++)
	{
		...
		saved_sites[i].coord.x = xValues[i];
		saved_sites[i].coord.y = yValues[i];
		saved_sites[i].sitenbr = i;
		saved_sites[i].refcnt = 0;
...
struct VoronoiDiagramGenerator::Halfedge* VoronoiDiagramGenerator::HEcreate(struct Edge *e,int pm)
{
...
struct VoronoiDiagramGenerator::Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct PointVDG *p)
{
	int i, bucket;
	struct Halfedge *he;
...
struct VoronoiDiagramGenerator::Halfedge * VoronoiDiagramGenerator::ELright(struct Halfedge *he)
{
	return (he -> ELright);
}

struct VoronoiDiagramGenerator::Halfedge * VoronoiDiagramGenerator::ELleft(struct Halfedge *he)
{
	return (he -> ELleft);
}


struct VoronoiDiagramGenerator::Site * VoronoiDiagramGenerator::leftreg(struct Halfedge *he)
{
...
struct VoronoiDiagramGenerator::Site * VoronoiDiagramGenerator::rightreg(struct Halfedge *he)
{
...
struct VoronoiDiagramGenerator::Edge * VoronoiDiagramGenerator::bisect(struct Site *s1,struct Site *s2)
{
	VFLOAT dx,dy,adx,ady;
	struct Edge *newedge;
	...
	ady = dy>0 ? dy : -dy;
	newedge -> c = (VFLOAT)(s1->coord.x * dx + s1->coord.y * dy + (dx*dx + dy*dy)*0.5);//get the slope of the line
...
struct VoronoiDiagramGenerator::Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct PointVDG *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	VFLOAT d, xint, yint;
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct PointVDG *p)
{
	...
	VFLOAT dxp, dyp, dxs, t1, t2, t3, yl;
...
VFLOAT VoronoiDiagramGenerator::dist(struct Site *s,struct Site *t)
{
	VFLOAT dx,dy;
	dx = s->coord.x - t->coord.x;
	dy = s->coord.y - t->coord.y;
	return (VFLOAT)(sqrt(dx*dx + dy*dy));
}
...
void VoronoiDiagramGenerator::PQinsert(struct Halfedge *he,struct Site * v, VFLOAT offset)
{
	struct Halfedge *last, *next;
	
	he -> vertex = v;
	ref(v);
	he -> ystar = (VFLOAT)(v -> coord.y + offset);
...
struct VoronoiDiagramGenerator::PointVDG VoronoiDiagramGenerator::PQ_min()
{
...
void VoronoiDiagramGenerator::pushGraphEdge(VFLOAT x1, VFLOAT y1, VFLOAT x2, VFLOAT y2, long s1, long s2)
{
	if(genVoronoi)
	{
...
void VoronoiDiagramGenerator::pushDelaunayGraphEdge(VFLOAT x1, VFLOAT y1, VFLOAT x2, VFLOAT y2)
{
...
void VoronoiDiagramGenerator::line(VFLOAT x1, VFLOAT y1, VFLOAT x2, VFLOAT y2)
{	
	pushGraphEdge(x1,y1,x2,y2);

}
void VoronoiDiagramGenerator::circle(VFLOAT x, VFLOAT y, VFLOAT radius){}
void VoronoiDiagramGenerator::range(VFLOAT minX, VFLOAT minY, VFLOAT maxX, VFLOAT maxY){}
#endif
...
void VoronoiDiagramGenerator::plotinit()
{
	...
	dy = ymax - ymin;
	dx = xmax - xmin;
	d = (VFLOAT)(( dx > dy ? dx : dy) * 1.1);
	pxmin = (VFLOAT)(xmin - (d-dx)/2.0);
	pxmax = (VFLOAT)(xmax + (d-dx)/2.0);
	pymin = (VFLOAT)(ymin - (d-dy)/2.0);
	pymax = (VFLOAT)(ymax + (d-dy)/2.0);
	cradius = (VFLOAT)((pxmax - pxmin)/350.0);
	openpl();
...
{
	struct Site *s1, *s2;
	VFLOAT x1=0,x2=0,y1=0,y2=0, temp = 0;
	Site *v1= 0, *v2 = 0;
...
//	
		...
		(y1 == y2 && y2 == pymin) || (y1 == y2 && y2 == pymax)))
	{
		...
		pushGraphEdge(x1,y1,x2,y2, e->reg[0]->sitenbr, e->reg[1]->sitenbr);
	}
}
...
bool VoronoiDiagramGenerator::getNextVertexPair(VFLOAT& x1, VFLOAT& y1, VFLOAT& x2, VFLOAT& y2)
{
	if(finalVertexLinks == 0)
		return false;
...
int VoronoiDiagramGenerator::scomp(const void *p1,const void *p2)
{
