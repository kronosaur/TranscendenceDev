...
//	Ctx.iDamage at the beginning and at the end.
	...
	if (Ctx.IsParalyzed())
		pSource->SetConditionDueToDamage(Ctx, ECondition::paralyzed);

	//	If this is blinding damage then our sensors are disabled

	if (Ctx.IsBlinded())
		pSource->SetConditionDueToDamage(Ctx, ECondition::blind);

	//	If this attack is radioactive, then contaminate the ship

	if (Ctx.IsRadioactive())
		pSource->SetConditionDueToDamage(Ctx, ECondition::radioactive);
...
//	attributes--enhancements are added later by the caller.
	...
	if (ArmorItem.GetRepairLevel() != Stats.iLevel)
		retList->Insert(SDisplayAttribute(attribNeutral, strPatternSubst(CONSTLIT("repair level %d"), ArmorItem.GetRepairLevel())));
	...
	if (iDmgSystem == EDamageMethodSystem::dmgMethodSysPhysicalized)
		...
		for (int i = 0; i < PHYSICALIZED_DAMAGE_METHOD_COUNT; i++)
			...
			if (IS_NAN(m_Fortification.Get(iMethod)))
				rMethodFortificationAdj += g_pUniverse->GetEngineOptions().GetDmgMethodFortificationItemArmor(iMethod);
			else
		...
		if (IS_NAN(m_Fortification.GetWMD()))
			rFortificationAdj += g_pUniverse->GetEngineOptions().GetDmgMethodFortificationItemArmor(iMethod);
		else
...
//	GetRepairCost
//
//	Returns the repair cost in default currency.
...
//	Returns the tech level required to repair this armor segment.
	...
	int iActualLevel = ArmorItem.GetLevel();
	int iBaseLevel = ArmorItem.GetMinLevel();
