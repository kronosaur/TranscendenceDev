...
Metric CStationHullDesc::GetFortificationAdj(EDamageMethod iMethod) const
	{
	if (IS_NAN(m_Fortified.Get(iMethod)))
		{
		...
		return m_Fortified.Get(iMethod);
	}
...
//	Initializes from a <Hull> element.
	...
	if (iDmgSystem == EDamageMethodSystem::dmgMethodSysPhysicalized)
		...
		if (bHasPhysicalizedFortify)
			...
			m_Fortified.SetPierce(pDesc->GetAttributeDoubleDefault(FORTIFICATION_WMD_ATTRIB, R_NAN));
		...
		if (bHasWMDFortify)
			...
			m_Fortified.SetWMD(pDesc->GetAttributeDoubleDefault(FORTIFICATION_PIERCE_ATTRIB, R_NAN));
		else
