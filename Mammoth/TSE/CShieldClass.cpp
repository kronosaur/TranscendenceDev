...
#define HEAT_ADJ_PER_CHARGE_ATTRIB				CONSTLIT("heatBonusPerCharge")
#define HEAT_DAMAGE_CONVERSION_ATTRIB			CONSTLIT("heatDamageConversion")
#define HEAT_GENERATION_ATTRIB					CONSTLIT("heatGeneration")
#define HEAT_PENALTY_THRESHOLD_ATTRIB			CONSTLIT("heatPenaltyThreshold")
#define HEAT_THRESHOLD_ZERO_HP_ATTRIB			CONSTLIT("heatThresholdZeroHP")
#define HIT_EFFECT_ATTRIB						CONSTLIT("hitEffect")
#define HIT_POINTS_ATTRIB						CONSTLIT("hitPoints")
#define IDLE_HEAT_GENERATION_ATTRIB				CONSTLIT("idleHeatGeneration")
...
#define PROPERTY_HEAT_DAMAGE_CONVERSION			CONSTLIT("heatDamageConversion")
...
//	NOTE: We always set Ctx.iAbsorb properly, regardless of the return value.
	...
	if ((Ctx.iAbsorb || Ctx.IsShotReflected())
		...
		if (m_pHitEffect)
			{
			CreateHitEffect(*pDevice, *pShip, Ctx, *m_pHitEffect, Ctx.vHitPos);
			}

		if (m_pFlashEffect && pDevice->IsReady())
			{
			CreateHitEffect(*pDevice, *pShip, Ctx, *m_pFlashEffect, pShip->GetPos());
	...
	if (Ctx.iShieldDamage > 0)
		...
		if (Ctx.iShieldDamage >= Ctx.iHPLeft)
			...
			if (m_iHeatDamageConversion > 0)
				pShip->SetHeatValue(pShip->GetHeatValue() + (m_iHeatDamageConversion * Ctx.iShieldDamage));
			}
...
int CShieldClass::CalcHeatDelta (const SUpdateCtx& Ctx, const CInstalledDevice* pDevice, CSpaceObject* pSource)

//	CalcHeatDelta
//
//	Returns the heat generated by the shield for this tick. Does not include heat generated by taking damage.
	{
	CItemCtx ItemCtx(pSource, pDevice);
...
//	Creates from an XML element
	...
	pShield->m_iHeatGeneration = pDesc->GetAttributeIntegerBounded(HEAT_GENERATION_ATTRIB, 0, -1, 0);
	pShield->m_iIdlePowerUse = pDesc->GetAttributeIntegerBounded(IDLE_POWER_USE_ATTRIB, 0, -1, pShield->m_iPowerUse / 2);
	pShield->m_iIdleHeatGeneration = pDesc->GetAttributeIntegerBounded(IDLE_HEAT_GENERATION_ATTRIB, 0, -1, pShield->m_iHeatGeneration / 2);
	pShield->m_iHeatDamageConversion = pDesc->GetAttributeIntegerBounded(HEAT_DAMAGE_CONVERSION_ATTRIB, 0, -1, 0);
	pShield->m_iHeatPenaltyThreshold = pDesc->GetAttributeIntegerBounded(HEAT_PENALTY_THRESHOLD_ATTRIB, 0, -1, 0);
	pShield->m_iHeatThresholdZeroHP = pDesc->GetAttributeIntegerBounded(HEAT_THRESHOLD_ZERO_HP_ATTRIB, 0, -1, 0);
	...
	pShield->m_iExtraHeatPerCharge = pDesc->GetAttributeInteger(HEAT_ADJ_PER_CHARGE_ATTRIB);
	...
	if (error = pShield->m_pFlashEffect.LoadEffect(Ctx,
			strPatternSubst(CONSTLIT("%d:f"), InitCtx.pType->GetUNID()),
...
void CShieldClass::CreateHitEffect (CInstalledDevice &Device, CSpaceObject &Ship, SDamageCtx &DamageCtx, CEffectCreator &Effect, const CVector &vPos) const

//	CreateHitEffect
//
//	Creates the given hit effect.

	{
	//	Create the hit effect painter.

	CSystem *pSystem = (Ship.GetSystem());
	if (!pSystem)
		return;
	...
	Options.pAnchor = (!Ship.IsDestroyed() ? &Ship : NULL);
...
int CShieldClass::GetHeatRating (CItemCtx& Ctx, int* retiIdleHeatGeneration) const
	{
	int iHeat = m_iHeatGeneration;
	int iIdleHeat = m_iIdleHeatGeneration;

	if (m_iExtraHeatPerCharge)
	{
		iHeat += m_iExtraHeatPerCharge * Ctx.GetItemCharges();

		if (m_iPowerUse > 0)
		{
			Metric rAdj = (Metric)m_iIdlePowerUse / m_iPowerUse;
			iIdleHeat += mathRound(m_iExtraHeatPerCharge * rAdj * Ctx.GetItemCharges());
		}
		else
			iIdleHeat += (m_iExtraHeatPerCharge * Ctx.GetItemCharges()) / 2;
	}
	...
	if (pEnhancements)
	{
		...
		iHeat = iHeat * iAdj / 100;
		iIdleHeat = iIdleHeat * iAdj / 100;
	}

	if (retiIdleHeatGeneration)
		*retiIdleHeatGeneration = iIdleHeat;
