...
ALERROR CCloudService::RequestExtensionDownload (DWORD dwUNID, const CString &sFilePath, const CString &sFilespec, const CIntegerIP &FileDigest)
...
//	Download an extension file.
	...
	for (i = 0; i < m_Services.GetCount(); i++)
		if (m_Services[i]->IsEnabled() && m_Services[i]->HasCapability(ICIService::canDownloadExtension))
			{
			//	Only one service can register a user
			return m_Services[i]->RequestExtensionDownload(dwUNID, sFilePath, sFilespec, FileDigest);
			}
