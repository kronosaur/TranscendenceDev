...
ALERROR CTopologyNode::AddStargate (const CString &sName, const CString &sDestNode, const CString &sDestEntryPoint, const SStargateRouteDesc &RouteDesc)
...
//	Adds a new stargate to the topology
	...
	pDesc->MidPoints = RouteDesc.MidPoints;
...
//	Adds a new stargate to the topology
	...
	AddStargate(sGateID, sDestNodeID, sDestGateID, SStargateRouteDesc());
...
//	The names of the stargates are optionally passed in (if not, they are autogenerated).
	...
	bool bExists = (!sToName.IsBlank() && (pDestNode->m_NamedGates.GetAt(sDestGate) != NULL));
...
ALERROR CTopologyNode::CreateStargateRoute (const SStargateRouteDesc &Desc)
...
//	Creates a stargate connection
	...
	if (error = Desc.pFromNode->AddStargate(sSourceGate, Desc.pToNode->GetID(), sDestGate, Desc))
		return ERR_FAIL;

	//	Check to see if the destination gate exists already
	//	[If sToName is blank, then we know that the gate does not exist because we autogenerated it.]

	bool bExists = (!Desc.sToName.IsBlank() && (Desc.pToNode->m_NamedGates.GetAt(sDestGate) != NULL));
	...
	if (!bExists && !Desc.bOneWay && !Desc.pToNode->IsEndGame())
		{
		if (error = Desc.pToNode->AddStargate(sDestGate, Desc.pFromNode->GetID(), sSourceGate, Desc))
			return ERR_FAIL;
		}
