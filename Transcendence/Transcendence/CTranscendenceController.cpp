...
//	Handle commands from sessions, etc.
	...
	//	Choose a save file to load
		...
		{
		DisplayMultiverseStatus(NULL_STR);
		if (error = m_HI.OpenPopupSession(new CLoadGameSession(m_HI, m_Service, m_Model.GetSaveFileFolders(), m_Settings)))
			{
	...
	//	Show galactic map
		...
		CGalacticMapSession *pSession = new CGalacticMapSession(m_SessionCtx, m_Model.GetSystemMapThumbnails(), m_pGameSession->GetGalacticMapSettings());
