...
CLoadGameSession::CLoadGameSession (CHumanInterface &HI, CCloudService &Service, const TArray<CString> &Folders, CGameSettings &Settings) : IHISession(HI),
		m_Service(Service),
		m_Folders(Folders),
		m_Settings(Settings)
...
//	Initialize
	...
	CListSaveFilesTask::SOptions Options;
	Options.sUsername = m_Service.GetUsername();
	Options.cxWidth = SAVE_ENTRY_WIDTH;
	Options.bFilterPermadeath = m_Settings.GetBoolean(CGameSettings::forcePermadeath);
	Options.bDebugSaveFiles = m_Settings.GetBoolean(CGameSettings::debugSaveFiles);

	m_HI.AddBackgroundTask(new CListSaveFilesTask(m_HI, m_Folders, Options), 0, this, CMD_READ_COMPLETE);
