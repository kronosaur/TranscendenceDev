...
void CLinePainter::Rasterize (CG32bitImage &Image, int x1, int y1, int x2, int y2, int iWidth, TArray<CGRasterize::SLinePixel> *retPixels)
	...
	switch (CalcIntermediates(Image, x1, y1, x2, y2, iWidth))
		...
		case lineXDominant:
			...
			for (x = m_xStart, y = m_yStart - m_iAxisHalfWidth - 1; x != m_xEnd; x += m_sx)
				...
				if (m_rWUp > 0.0 && w >= m_wMin && w < m_wMax)
					{
					CGRasterize::SLinePixel &Pixel = retPixels->GetAt(iPixel++);
					Pixel.pPos = pPos;
					Pixel.byAlpha = (BYTE)(DWORD)(255.0 * (m_rWUp > 1.0 ? 1.0 : m_rWUp));
					Pixel.x = x;
					Pixel.y = w;
					Pixel.rV = rInnerV;
					Pixel.rW = rInnerW;
				...
				if (m_rWDown > 0 && w >= m_wMin && w < m_wMax)
					...
					CGRasterize::SLinePixel &Pixel = retPixels->GetAt(iPixel++);
					Pixel.pPos = pPos;
					Pixel.byAlpha = (BYTE)(DWORD)(255.0 * (m_rWDown > 1.0 ? 1.0 : m_rWDown));
					Pixel.x = x;
					Pixel.y = w;
